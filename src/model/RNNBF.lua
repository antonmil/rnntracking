...
function RNN.rnn(opt)
  ...
  local dynSmoothLoss = opt.omicron ~= 0
...
-- if exVar then error('n  
  ...
  local stateInpInd = 1
  local detInpInd = n*nHiddenInputs+2
...
--   print(xSize)
  ...
  local splitModel = opt.model_split ~= 0
  if splitModel then
...
--     batchMode=inputs[1]:size(1)>1
  ...
  for L=1, n do
    ...
    local prev_h = inputs[nHiddenInputs*L+1] -- inputs[1] is current location
    ...
    if L == 1 then
      ...
      x = pred_state[(L-1)*nHiddenInputs]
  ...
  local projState = nn.Linear(h1Size, xSize)(top_pred_state):annotate{
      name='state X*_t',
...
--     end
    dp = nn.Reshape(xSize, batchMode)(dp)
    detInp = nn.Linear(xSize,h1Size)(dp):annotate{name='Det Inp'}
...
--       graphAttributes = {color='red'}}
  ...
  local projStateUpdate = nn.Linear(h1Size, xSize)(top_h_Upd_state):annotate{
      name='state X_t',
...
--       local exVec = nn.Replicate(opt.state_dim,2,0)(nn.Reshape(opt.max_n, 1)(inputs[exInpInd]))
--       dp = nn.CMulTable(){exVec,dp}
    ter = nn.Linear(maxTargets, h2Size)(inputs[exInpInd])
    if splitModel then 
...
--     top_Ter_state = nn.CAddTable(){nn.Linear(maxTargets*nClasses, h2Size)(nn.Reshape(maxTargets*nClasses, batchMode)(LInp)), ter}
    top_Ter_state = nn.CAddTable(){nn.Linear(maxTargets, h2Size)(nn.Select(3,nClasses)(nn.Reshape(maxTargets,nClasses, batchMode)(LInp))), ter}
    top_Ter_state = nn.Reshape(h2Size, batchMode)(top_Ter_state)
    ter = nn.Linear(h2Size, maxTargets)(top_Ter_state)
    ter = nn.Sigmoid()(ter)
